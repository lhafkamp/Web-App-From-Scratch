  ...
  /** GOOGLE MAPS FUNCTIONS
   ...
   var gMapsModule = {
     ...
     generate_map: function(myOptions, canvasID) {
       ...
       for (var i = 0; i < locaties.length; i++) {
         try {
             ...
             debug_message("Localstorage can't be reached: " + error);
         }
         var markerLatLng = new google.maps.LatLng(locaties[i][3], locaties[i][4]);
         routeList.push(markerLatLng);

         varArray.markerArray[i] = {};
         for (var attr in locatieMarker) {
             varArray.markerArray[i][attr] = locatieMarker[attr];
         }
         ...
         var marker = new google.maps.Marker({
             ...
             map: varArray.map,
             icon: varArray.markerArray[i],
             title: locaties[i][0]
         });
       }

       if(tourType == varArray.lineair){
           // Draw lines between the points
           debug_message("Drawing route");
       ...
       varArray.currentPositionMarker = new google.maps.Marker({
       ...
       ET.addListener(varArray.positionUpdated, update_positie);
     ...
     update_positie: function(event) {
       ...
       var newPos = new google.maps.LatLng(varArray.currentPosition.coords.latitude, varArray.currentPosition.coords.longitude);
       varArray.map.setCenter(newPos);
       varArray.currentPositionMarker.setPosition(newPos);
     }
   ...
   var debugModule = {
     ...
     debug_message: function(message) {
       (varArray.customDebugging && varArray.debugId) ? document.getElementById(varArray.debugId).innerHTML : console.log(message);
     },

     set_custom_debugging: function(debugId) {
       varArray.debugId = this.debugId;
       varArray.customDebugging = true;
     }
